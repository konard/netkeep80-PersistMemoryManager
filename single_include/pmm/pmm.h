...
if (!_initialized || _backend.base_ptr() == nullptr) return nullptr;
detail::ManagerHeader<address_traits> *hdr = get_header(_backend.base_ptr());
if (hdr->root_offset == address_traits::no_block || !is_valid_user_offset_unlocked(hdr->root_offset, sizeof(forest_registry))) return nullptr;
auto *reg = reinterpret_cast<forest_registry *>( _backend.base_ptr() + static_cast<std::size_t>(hdr->root_offset) * address_traits::granule_size);
if (reg->magic != detail::kForestRegistryMagic || reg->version != detail::kForestRegistryVersion || reg->domain_count > detail::kMaxForestDomains) return nullptr;
return reg;
}
static forest_domain *find_domain_by_name_unlocked(const char *name) noexcept {
if (!detail::forest_domain_name_fits(name)) return nullptr;
forest_registry *reg = forest_registry_root_unlocked();
if (reg == nullptr) return nullptr;
for (std::uint16_t i = 0; i < reg->domain_count; ++i) {
if (detail::forest_domain_name_equals(reg->domains[i], name)) return &reg->domains[i];
}
return nullptr;
}
static forest_domain * find_domain_by_binding_unlocked(index_type binding_id) noexcept {
if (binding_id == 0) return nullptr;
forest_registry *reg = forest_registry_root_unlocked();
if (reg == nullptr) return nullptr;
for (std::uint16_t i = 0; i < reg->domain_count; ++i) {
if (reg->domains[i].binding_id == binding_id) return &reg->domains[i];
}
return nullptr;
}
static forest_domain * find_domain_by_symbol_unlocked(pptr<pstringview> symbol) noexcept {
if (symbol.is_null()) return nullptr;
const char *sym_str = pstringview_c_str_unlocked(symbol);
if (sym_str == nullptr) return nullptr;
forest_registry *reg = forest_registry_root_unlocked();
if (reg == nullptr) return nullptr;
for (std::uint16_t i = 0; i < reg->domain_count; ++i) {
if (reg->domains[i].symbol_offset == symbol.offset() || std::strncmp(reg->domains[i].name, sym_str, detail::kForestDomainNameCapacity) == 0) {
...
const detail::ManagerHeader<address_traits> *hdr = (_backend.base_ptr() != nullptr) ? get_header_c(_backend.base_ptr()) : nullptr;
if (rec == nullptr || hdr == nullptr) return 0;
if (rec->binding_kind == detail::kForestBindingFreeTree) return (hdr->free_tree_root == address_traits::no_block) ? static_cast<index_type>(0) : hdr->free_tree_root;
return rec->root_offset;
}
static index_type * forest_domain_root_index_ptr_unlocked(forest_domain *rec) noexcept {
if (rec == nullptr || rec->binding_kind != detail::kForestBindingDirectRoot) return nullptr;
return &rec->root_offset;
}
...
static bool register_domain_unlocked(const char *name, std::uint8_t flags, std::uint8_t binding_kind, index_type initial_root) noexcept {
if (!detail::forest_domain_name_fits(name)) return false;
forest_registry *reg = forest_registry_root_unlocked();
if (reg == nullptr) return false;
if (forest_domain *existing = find_domain_by_name_unlocked(name)) {
existing->flags |= flags;
existing->binding_kind = binding_kind;
if (binding_kind == detail::kForestBindingDirectRoot && initial_root != 0) existing->root_offset = initial_root;
if (existing->symbol_offset == 0) {
pptr<pstringview> symbol = intern_symbol_unlocked(name);
if (!symbol.is_null()) existing->symbol_offset = symbol.offset();
}
return true;
}
if (reg->domain_count >= detail::kMaxForestDomains) return false;
forest_domain rec{
};
if (!detail::forest_domain_name_copy(rec, name)) return false;
...
rec.symbol_offset = 0;
pptr<pstringview> symbol = intern_symbol_unlocked(name);
if (!symbol.is_null()) rec.symbol_offset = symbol.offset();
...
if (symbol_policy.root_index_ptr() == nullptr) return pptr<pstringview>();
...
if (raw == nullptr) return pptr<pstringview>();
...
detail::avl_init_node(new_node);
if (!lock_block_permanent_unlocked(public_raw)) return pptr<pstringview>();
...
detail::kSystemDomainFreeTree, detail::kSystemDomainSymbols, detail::kSystemDomainRegistry, detail::kSystemTypeForestRegistry, detail::kSystemTypeForestDomainRecord, detail::kSystemTypePstringview, detail::kServiceNameDomainRoot, detail::kServiceNameDomainSymbol, };
for (const char *sym : kBootstrapSymbols) {
if (intern_symbol_unlocked(sym).is_null()) return false;
}
forest_registry *reg = forest_registry_root_unlocked();
if (reg == nullptr) return false;
for (std::uint16_t i = 0; i < reg->domain_count; ++i) {
if (reg->domains[i].name[0] == '\0') continue;
if (reg->domains[i].symbol_offset != 0) continue;
...
if (raw == nullptr) {
if (_last_error == PmmError::Ok) _last_error = PmmError::OutOfMemory;
return false;
}
std::uint8_t *base = _backend.base_ptr();
...
get_header(_backend.base_ptr())->root_offset = detail::ptr_to_granule_idx<address_traits>(_backend.base_ptr(), reg);
if (!register_domain_unlocked(detail::kSystemDomainFreeTree, detail::kForestDomainFlagSystem, detail::kForestBindingFreeTree, 0)) {
_last_error = PmmError::BackendError;
return false;
}
if (!register_domain_unlocked(detail::kSystemDomainSymbols, detail::kForestDomainFlagSystem, detail::kForestBindingDirectRoot, 0)) {
_last_error = PmmError::BackendError;
return false;
}
if (!register_domain_unlocked(detail::kSystemDomainRegistry, detail::kForestDomainFlagSystem, detail::kForestBindingDirectRoot, get_header(_backend.base_ptr())->root_offset)) {
_last_error = PmmError::BackendError;
return false;
}
if (!register_domain_unlocked(detail::kServiceNameDomainRoot, detail::kForestDomainFlagSystem, detail::kForestBindingDirectRoot, 0)) {
_last_error = PmmError::BackendError;
return false;
}
...
if (hdr->image_version != detail::kCurrentImageVersion) return false;
if (hdr->total_size != _backend.total_size()) return false;
if (hdr->granule_size != static_cast<std::uint16_t>(address_traits::granule_size)) return false;
const forest_registry *reg = forest_registry_root_unlocked();
if (reg == nullptr) return false;
if (reg->magic != detail::kForestRegistryMagic) return false;
if (reg->version != detail::kForestRegistryVersion) return false;
if (reg->domain_count < 4) return false;
static constexpr const char *kRequired[] = {
detail::kSystemDomainFreeTree, detail::kSystemDomainSymbols, detail::kSystemDomainRegistry, detail::kServiceNameDomainRoot};
for (const char *name : kRequired) {
const forest_domain *rec = find_domain_by_name_unlocked(name);
if (rec == nullptr) return false;
if ((rec->flags & detail::kForestDomainFlagSystem) == 0) return false;
if (rec->symbol_offset == 0) return false;
}
const forest_domain *free_rec = find_domain_by_name_unlocked(detail::kSystemDomainFreeTree);
if (free_rec->binding_kind != detail::kForestBindingFreeTree) return false;
if (pstringview::forest_domain_ops().root_index() == 0) return false;
const forest_domain *reg_rec = find_domain_by_name_unlocked(detail::kSystemDomainRegistry);
if (reg_rec->root_offset != hdr->root_offset) return false;
const forest_domain *root_rec = find_domain_by_name_unlocked(detail::kServiceNameDomainRoot);
if (root_rec->binding_kind != detail::kForestBindingDirectRoot) return false;
return true;
}
static bool validate_or_bootstrap_forest_registry_unlocked() noexcept {
detail::ManagerHeader<address_traits> *hdr = get_header(_backend.base_ptr());
if (forest_registry_root_unlocked() != nullptr) {
if (!register_domain_unlocked(detail::kSystemDomainFreeTree, detail::kForestDomainFlagSystem, detail::kForestBindingFreeTree, 0)) return false;
if (!register_domain_unlocked( detail::kSystemDomainSymbols, detail::kForestDomainFlagSystem, detail::kForestBindingDirectRoot, pstringview::forest_domain_ops().root_index())) return false;
if (!register_domain_unlocked( detail::kSystemDomainRegistry, detail::kForestDomainFlagSystem, detail::kForestBindingDirectRoot, hdr->root_offset)) return false;
if (!register_domain_unlocked(detail::kServiceNameDomainRoot, detail::kForestDomainFlagSystem, detail::kForestBindingDirectRoot, 0)) return false;
return bootstrap_system_symbols_unlocked();
}
hdr->root_offset = address_traits::no_block;
return bootstrap_forest_registry_unlocked();
}
template <typename Callback> static void for_each_free_block_inorder( const std::uint8_t *base, const detail::ManagerHeader<address_traits> *hdr, index_type node_idx, int depth, Callback &&callback) noexcept {
using BlockState = BlockStateBase<address_traits>;
static constexpr std::size_t kGranSz = address_traits::granule_size;
if (node_idx == address_traits::no_block) return;
if (static_cast<std::size_t>(node_idx) * kGranSz + sizeof(Block<address_traits>) > hdr->total_size) return;
...
view.left_offset = (left_off != address_traits::no_block) ? static_cast<std::ptrdiff_t>( static_cast<std::size_t>(left_off) * kGranSz) : -1;
view.right_offset = (right_off != address_traits::no_block) ? static_cast<std::ptrdiff_t>( static_cast<std::size_t>(right_off) * kGranSz) : -1;
view.parent_offset = (parent_off != address_traits::no_block) ? static_cast<std::ptrdiff_t>( static_cast<std::size_t>(parent_off) * kGranSz) : -1;
...
constexpr std::size_t rounded_header_size = static_cast<std::size_t>(kBlockHdrGranules) * address_traits::granule_size;
constexpr std::size_t min_public_user_offset = static_cast<std::size_t>(kFreeBlkIdxLayout + kBlockHdrGranules) * address_traits::granule_size;
...
if ((static_cast<std::size_t>(cand - base) % address_traits::granule_size) == 0 && cand + sizeof(Block<address_traits>) <= base + static_cast<std::size_t>(hdr->total_size) && detail::is_canonical_allocated_block_header<address_traits>( base, static_cast<std::size_t>(hdr->total_size), cand)) return reinterpret_cast<const pmm::Block<address_traits> *>(cand);
}
}
}
return detail::header_from_ptr_t<address_traits>( const_cast<std::uint8_t *>(base), const_cast<void *>(ptr), static_cast<std::size_t>(hdr->total_size));
}
template <typename T> static pptr<T> make_pptr_from_raw(void *raw) noexcept {
if (raw == nullptr || !_initialized) return pptr<T>();
std::uint8_t *base = _backend.base_ptr();
auto *raw_byte = static_cast<std::uint8_t *>(raw);
if (base == nullptr) return pptr<T>();
const std::uintptr_t base_addr = reinterpret_cast<std::uintptr_t>(base);
const std::uintptr_t raw_addr = reinterpret_cast<std::uintptr_t>(raw_byte);
if (raw_addr < base_addr) return pptr<T>();
const std::size_t raw_off = static_cast<std::size_t>(raw_addr - base_addr);
if (raw_off < sizeof(Block<address_traits>) || raw_off >= _backend.total_size()) return pptr<T>();
const std::size_t blk_off = raw_off - sizeof(Block<address_traits>);
if (blk_off % address_traits::granule_size != 0) return pptr<T>();
const std::size_t blk_idx_raw = blk_off / address_traits::granule_size;
if (blk_idx_raw > static_cast<std::size_t>(std::numeric_limits<index_type>::max())) return pptr<T>();
index_type blk_idx = static_cast<index_type>(blk_idx_raw);
if (!detail::validate_block_index<address_traits>(_backend.total_size(), blk_idx)) return pptr<T>();
const void *blk = detail::block_at<address_traits>(base, blk_idx);
if (BlockStateBase<address_traits>::get_weight(blk) == 0 || BlockStateBase<address_traits>::get_root_offset(blk) != blk_idx) return pptr<T>();
const std::uint16_t node_type = BlockStateBase<address_traits>::get_node_type(blk);
if (node_type != pmm::kNodeReadWrite && node_type != pmm::kNodeReadOnly) return pptr<T>();
if (blk_idx > std::numeric_limits<index_type>::max() - kBlockHdrGranules) return pptr<T>();
...
const std::uint8_t *base = _backend.base_ptr();
if (p.offset() < kBlockHdrGranules) return nullptr;
std::size_t blk_off = static_cast<std::size_t>(p.offset() - kBlockHdrGranules) * address_traits::granule_size;
if (blk_off + sizeof(Block<address_traits>) > _backend.total_size()) return nullptr;
return base + blk_off;
}
...
constexpr index_type kHdrGranules = static_cast<index_type>( (sizeof(Block<address_traits>) + address_traits::granule_size - 1) / address_traits::granule_size);
...
if (p.is_null() || !_initialized) return nullptr;
std::uint8_t *base = _backend.base_ptr();
std::size_t byte_off = static_cast<std::size_t>(p.offset()) * address_traits::granule_size;
if (byte_off + sizeof(T) > _backend.total_size()) return nullptr;
return base + byte_off;
}
...
constexpr index_type kHdrGranules = static_cast<index_type>( (sizeof(Block<address_traits>) + address_traits::granule_size - 1) / address_traits::granule_size);
if (p.offset() < kHdrGranules) return nullptr;
...
index_type idx = hdr->first_block_offset;
while (idx != address_traits::no_block) {
if (!detail::validate_block_index<address_traits>(hdr->total_size, idx)) break;
...
result.add(ViolationType::ForestRegistryMissing, DiagnosticAction::NoAction, 0, static_cast<std::uint64_t>(detail::kForestRegistryMagic), static_cast<std::uint64_t>(reg->magic));
return;
}
static constexpr const char *kRequired[] = {
detail::kSystemDomainFreeTree, detail::kSystemDomainSymbols, detail::kSystemDomainRegistry, detail::kServiceNameDomainRoot};
