...
public: using address_traits = typename ConfigT::address_traits;
...
std::size_t additional = (_backend.total_size() < aligned) ? (aligned - _backend.total_size()) : aligned;
...
if (ok) ok = bootstrap_forest_registry_unlocked();
if (ok) ok = validate_bootstrap_invariants_unlocked();
if (ok) {
_last_error = PmmError::Ok;
...
result.add(ViolationType::HeaderCorruption, DiagnosticAction::Aborted, 0, static_cast<std::uint64_t>(kMagic), static_cast<std::uint64_t>(hdr->magic));
return false;
}
...
result.add(ViolationType::HeaderCorruption, DiagnosticAction::Aborted, 0, detail::kCurrentImageVersion, static_cast<std::uint64_t>(hdr->image_version));
return false;
}
...
result.add(ViolationType::HeaderCorruption, DiagnosticAction::Aborted, 0, _backend.total_size(), static_cast<std::uint64_t>(hdr->total_size));
return false;
}
...
result.add(ViolationType::HeaderCorruption, DiagnosticAction::Aborted, 0, address_traits::granule_size, static_cast<std::uint64_t>(hdr->granule_size));
return false;
}
auto mark_entries = [](VerifyResult &r, std::size_t from, DiagnosticAction act) {
for (std::size_t i = from; i < r.entry_count; ++i) r.entries[i].action = act;
};
...
mark_entries(result, pre, DiagnosticAction::Rebuilt);
if (detail::image_version_requires_migration(hdr->image_version)) hdr->image_version = detail::kCurrentImageVersion;
...
if (result.entries[i].type == ViolationType::ForestRegistryMissing || result.entries[i].type == ViolationType::ForestDomainMissing || result.entries[i].type == ViolationType::ForestDomainFlagsMissing) result.entries[i].action = DiagnosticAction::Aborted;
}
_initialized = false;
return false;
}
...
typename thread_policy::unique_lock_type lock(_mutex);
std::uint8_t *base = _backend.base_ptr();
if (base != nullptr && _backend.total_size() >= detail::kMinMemorySize) get_header(base)->magic = 0;
_initialized = false;
logging_policy::on_destroy();
}
...
if (!_initialized || ptr == nullptr) return false;
...
typename thread_policy::unique_lock_type lock(_mutex);
if (!_initialized) return;
set_forest_domain_root_index_unlocked( find_domain_by_name_unlocked(detail::kServiceNameDomainRoot), p.is_null() ? static_cast<index_type>(0) : p.offset());
}
template <typename T> static pptr<T> get_root() noexcept {
typename thread_policy::shared_lock_type lock(_mutex);
if (!_initialized) return pptr<T>();
index_type root = forest_domain_root_index_unlocked( find_domain_by_name_unlocked(detail::kServiceNameDomainRoot));
if (root == static_cast<index_type>(0)) return pptr<T>();
return pptr<T>(root);
}
...
return set_forest_domain_root_index_unlocked( rec, root.is_null() ? static_cast<index_type>(0) : root.offset());
}
private: template <typename T, typename ValueT> static ValueT get_tree_field(pptr<T> p, ValueT (*getter)(const void *)) noexcept {
if (p.is_null() || !_initialized) return ValueT{
};
...
template <typename T, typename ValueT> static void set_tree_field(pptr<T> p, void (*setter)(void *, ValueT), ValueT value) noexcept {
if (p.is_null() || !_initialized) return;
...
template <typename T> static index_type get_tree_idx_field(pptr<T> p, index_type (*getter)(const void *)) noexcept {
index_type v = get_tree_field(p, getter);
return (v == address_traits::no_block) ? static_cast<index_type>(0) : v;
}
template <typename T> static void set_tree_idx_field(pptr<T> p, void (*setter)(void *, index_type), index_type val) noexcept {
set_tree_field(p, setter, (val == 0) ? address_traits::no_block : val);
}
public: template <typename T> static index_type get_tree_left_offset(pptr<T> p) noexcept {
...
private: template <typename Fn> static std::size_t read_stat(Fn fn) noexcept {
...
public: static std::size_t total_size() noexcept {
if (!_initialized.load(std::memory_order_acquire)) return 0;
typename thread_policy::shared_lock_type lock(_mutex);
...
return read_stat([](const auto *h) { std::size_t used = address_traits::granules_to_bytes(h->used_size); return (h->total_size > used) ? (h->total_size - used) : std::size_t(0); });
}
...
if (static_cast<std::size_t>(idx) * kGranSz + sizeof(Block<address_traits>) > hdr->total_size) break;
...
private: static inline storage_backend _backend{
};
static inline std::atomic<bool> _initialized{
false};
static inline typename thread_policy::mutex_type _mutex{
};
static inline thread_local PmmError _last_error{
PmmError::Ok};
static bool is_valid_user_offset_unlocked(index_type off, std::size_t size_bytes) noexcept {
if (off == 0 || _backend.base_ptr() == nullptr || _backend.total_size() == 0) return false;
...
if (!_initialized || ptr == nullptr) return;
...
if (freed == 0) return;
if (BlockStateBase<address_traits>::get_node_type(blk) == pmm::kNodeReadOnly) return;
std::uint8_t *base = _backend.base_ptr();
detail::ManagerHeader<address_traits> *hdr = get_header(base);
...
hdr->alloc_count--;
hdr->free_count++;
if (hdr->used_size >= freed) hdr->used_size -= freed;
...
static_assert(std::is_nothrow_constructible_v<T, Args...>, "create_typed_unlocked<T>: T must be nothrow-constructible");
void *raw = allocate_unlocked(sizeof(T));
if (raw == nullptr) return pptr<T>();
...
static constexpr index_type kBlockHdrGranules = static_cast<index_type>(kBlockHdrByteSize / address_traits::granule_size);
