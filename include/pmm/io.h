...
if (ok) ok = detail::flush_file_to_storage(f);
...
if (!detail::flush_parent_directory(filename)) return false;
return true;
}
...
if (buf == nullptr || size < detail::kMinMemorySize) return false;
...
std::fclose(f);
if (read_bytes != file_size) return false;
