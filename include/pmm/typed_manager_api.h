...
public: template <typename T> static pmm::pptr<T, ManagerT> allocate_typed() noexcept {
void *raw = ManagerT::allocate(sizeof(T));
if (raw == nullptr) return pmm::pptr<T, ManagerT>();
...
if (count == 0) return pmm::pptr<T, ManagerT>();
if (sizeof(T) > 0 && count > (std::numeric_limits<std::size_t>::max)() / sizeof(T)) return pmm::pptr<T, ManagerT>();
void *raw = ManagerT::allocate(sizeof(T) * count);
if (raw == nullptr) return pmm::pptr<T, ManagerT>();
return ManagerT::template make_pptr_from_raw<T>(raw);
}
template <typename T> static void deallocate_typed(pmm::pptr<T, ManagerT> p) noexcept {
if (p.is_null() || !ManagerT::_initialized) return;
...
*/
template <typename T> static pmm::pptr<T, ManagerT> reallocate_typed(pmm::pptr<T, ManagerT> p, std::size_t old_count, std::size_t new_count) noexcept {
...
static_assert(std::is_trivially_copyable_v<T>, "reallocate_typed<T>: T must be trivially copyable for safe " "memcpy reallocation.");
if (new_count == 0) {
ManagerT::_last_error = PmmError::InvalidSize;
return pmm::pptr<T, ManagerT>();
}
if (p.is_null()) return allocate_typed<T>(new_count);
...
if (new_data_gran == 0) new_data_gran = 1;
...
if (new_data_gran_alloc == 0) new_data_gran_alloc = 1;
...
hdr->alloc_count--;
hdr->free_count++;
if (hdr->used_size >= freed_w) hdr->used_size -= freed_w;
...
static_assert(std::is_nothrow_constructible_v<T, Args...>, "create_typed<T>: T must be nothrow-constructible from Args. " "Use allocate_typed<T>() + manual placement new for throwing " "constructors.");
void *raw = ManagerT::allocate(sizeof(T));
if (raw == nullptr) return pmm::pptr<T, ManagerT>();
...
T *obj = resolve_unchecked<T>(p);
void *raw = ManagerT::template raw_block_user_ptr_from_pptr<T>(p);
if (obj == nullptr || raw == nullptr) return;
obj->~T();
ManagerT::deallocate(raw);
}
...
if (p.is_null() || !ManagerT::_initialized) return nullptr;
...
index_type blk_idx = ManagerT::template block_idx_from_pptr<T>(p);
if (BlockStateBase<address_traits>::get_weight(blk_raw) == 0 || BlockStateBase<address_traits>::get_root_offset(blk_raw) != blk_idx) {
ManagerT::_last_error = PmmError::InvalidPointer;
return nullptr;
}
...
using address_traits = typename ManagerT::address_traits;
using index_type = typename ManagerT::index_type;
if (byte_off == 0) return pmm::pptr<T, ManagerT>();
