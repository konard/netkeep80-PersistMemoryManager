...
if (_data_idx == detail::kNullIdx_v<typename ManagerT::address_traits>) return "";
...
if (!ensure_capacity(len)) return false;
char *data = resolve_data();
...
if (new_len < _length) return false;
...
private: char *resolve_data() const noexcept {
return reinterpret_cast<char *>( detail::resolve_granule_ptr<typename ManagerT::address_traits>( ManagerT::backend().base_ptr(), _data_idx));
}
...
if (old_data != nullptr) std::memcpy(new_raw, old_data, static_cast<std::size_t>(_length) + 1);
} else {
