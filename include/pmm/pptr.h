...
template <typename ManagerT> requires requires {
typename ManagerT::address_traits::index_type;
}
struct manager_index_type<ManagerT> {
using type = typename ManagerT::address_traits::index_type;
};
}
template <class T, class ManagerT> requires(!std::is_void_v<ManagerT>)
/*
## pmm-pptr
*/
class pptr {
...
private: index_type _idx;
public: constexpr pptr() noexcept : _idx(0) {
}
...
constexpr index_type offset() const noexcept {
return _idx;
}
  /*
...
static_assert( requires(const T &a, const T &b) { { a < b } -> std::convertible_to<bool>; }, "pptr<T>::operator< requires T to support operator<. " "Provide bool operator<(const T&, const T&) or use pptr::offset() " "for index-based ordering.");
...
auto &tree_node() const noexcept {
return ManagerT::tree_node(*this);
}
};
}
