...
if (i >= static_cast<std::size_t>(_size)) return nullptr;
T *data = resolve_data();
...
if (!ensure_capacity(_size + 1)) return false;
T *d = resolve_data();
...
if (i >= static_cast<std::size_t>(_size)) return false;
T *d = resolve_data();
if (d == nullptr) return false;
d[i] = value;
return true;
...
if (new_size > _size) {
if (!ensure_capacity(new_size)) return false;
T *d = resolve_data();
if (d == nullptr) return false;
...
if (index > static_cast<std::size_t>(_size)) return false;
if (!ensure_capacity(_size + 1)) return false;
T *d = resolve_data();
if (d == nullptr) return false;
if (index < static_cast<std::size_t>(_size)) std::memmove(d + index + 1, d + index, (static_cast<std::size_t>(_size) - index) * sizeof(T));
d[index] = value;
++_size;
return true;
}
...
T *d = resolve_data();
if (d == nullptr) return false;
if (index + 1 < static_cast<std::size_t>(_size)) std::memmove(d + index, d + index + 1, (static_cast<std::size_t>(_size) - index - 1) * sizeof(T));
--_size;
return true;
}
void clear() noexcept {
...
ManagerT::deallocate( detail::resolve_granule_ptr<typename ManagerT::address_traits>( ManagerT::backend().base_ptr(), _data_idx));
...
private: T *resolve_data() const noexcept {
return reinterpret_cast<T *>( detail::resolve_granule_ptr<typename ManagerT::address_traits>( ManagerT::backend().base_ptr(), _data_idx));
}
bool ensure_capacity(std::uint32_t required) noexcept {
if (required <= _capacity) return true;
std::uint32_t new_cap = _capacity * 2;
if (new_cap < required) new_cap = required;
if (new_cap < 4) new_cap = 4;
std::size_t alloc_size = static_cast<std::size_t>(new_cap) * sizeof(T);
if (sizeof(T) > 0 && alloc_size / sizeof(T) != static_cast<std::size_t>(new_cap)) return false;
void *new_raw = ManagerT::allocate(alloc_size);
if (new_raw == nullptr) return false;
...
if (old_data != nullptr) std::memcpy(new_raw, old_data, static_cast<std::size_t>(_size) * sizeof(T));
}
if (_data_idx != detail::kNullIdx_v<typename ManagerT::address_traits>) ManagerT::deallocate( detail::resolve_granule_ptr<typename ManagerT::address_traits>( base, _data_idx));
