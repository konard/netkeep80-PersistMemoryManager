...
template <typename PPtr, typename IndexType> static void avl_set_child(PPtr parent, PPtr old_child, PPtr new_child, IndexType &root_idx) noexcept {
if (parent.is_null()) {
root_idx = new_child.offset();
return;
}
PPtr left_of_parent = pptr_get_left(parent);
if (left_of_parent == old_child) pptr_set_left(parent, new_child);
else pptr_set_right(parent, new_child);
}
/*
...
template <typename PPtr, typename IndexType, typename NodeUpdateFn = AvlUpdateHeightOnly> static PPtr avl_rotate_right(PPtr y, IndexType &root_idx, NodeUpdateFn update_node = {}) noexcept {
...
pptr_set_parent(y, x);
pptr_set_left(y, b);
if (!b.is_null()) pptr_set_parent(b, y);
...
template <typename PPtr, typename IndexType, typename NodeUpdateFn = AvlUpdateHeightOnly> static PPtr avl_rotate_left(PPtr x, IndexType &root_idx, NodeUpdateFn update_node = {}) noexcept {
...
pptr_set_parent(x, y);
pptr_set_right(x, b);
if (!b.is_null()) pptr_set_parent(b, x);
...
template <typename PPtr, typename IndexType, typename NodeUpdateFn = AvlUpdateHeightOnly> static void avl_rebalance_up(PPtr p, IndexType &root_idx, NodeUpdateFn update_node = {}) noexcept {
while (!p.is_null()) {
update_node(p);
std::int16_t bf = avl_balance_factor(p);
if (bf > 1) {
PPtr left = pptr_get_left(p);
if (avl_balance_factor(left) < 0) avl_rotate_left(left, root_idx, update_node);
p = avl_rotate_right(p, root_idx, update_node);
} else if (bf < -1) {
PPtr right = pptr_get_right(p);
if (avl_balance_factor(right) > 0) avl_rotate_right(right, root_idx, update_node);
...
template <typename PPtr, typename IndexType, typename NodeUpdateFn = AvlUpdateHeightOnly> static void avl_remove(PPtr target, IndexType &root_idx, NodeUpdateFn update_node = {}) noexcept {
...
template <typename PPtr, typename IndexType, typename CompareThreeWayFn, typename ResolveFn> static PPtr avl_find(IndexType root_idx, CompareThreeWayFn &&compare_three_way, ResolveFn &&resolve) noexcept {
PPtr cur(root_idx);
while (!cur.is_null()) {
if (resolve(cur) == nullptr) break;
int cmp = compare_three_way(cur);
if (cmp == 0) return cur;
else if (cmp < 0) cur = pptr_get_left(cur);
else cur = pptr_get_right(cur);
}
return PPtr();
}
template <typename PPtr> static PPtr avl_inorder_successor(PPtr cur) noexcept {
if (cur.is_null()) return PPtr();
PPtr right = pptr_get_right(cur);
if (!right.is_null()) return avl_min_node(right);
while (true) {
PPtr parent = pptr_get_parent(cur);
if (parent.is_null()) return PPtr();
PPtr parent_left = pptr_get_left(parent);
if (!parent_left.is_null() && parent_left.offset() == cur.offset()) return parent;
cur = parent;
}
}
...
template <typename PPtr, typename IndexType, typename GoLeftFn, typename ResolveFn, typename NodeUpdateFn = AvlUpdateHeightOnly> static void avl_insert(PPtr new_node, IndexType &root_idx, GoLeftFn &&go_left, ResolveFn &&resolve, NodeUpdateFn update_node = {}) noexcept {
if (new_node.is_null()) return;
if (resolve(new_node) == nullptr) return;
...
parent = cur;
left = go_left(cur);
if (left) cur = pptr_get_left(cur);
else cur = pptr_get_right(cur);
}
pptr_set_parent(new_node, parent);
if (left) pptr_set_left(parent, new_node);
else pptr_set_right(parent, new_node);
avl_rebalance_up(parent, root_idx, update_node);
}
template <typename Domain> concept ForestDomainViewDescriptor = requires(const Domain domain, typename Domain::node_pptr p) {
...
template <typename Domain> concept ForestDomainDescriptor = ForestDomainViewDescriptor<Domain> && requires(Domain domain, typename Domain::node_pptr p) {
{
domain.root_index_ptr() }
-> std::same_as<typename Domain::index_type *>;
{
domain.less_node(p, p) }
-> std::convertible_to<bool>;
};
template <typename Domain, typename Key> concept ForestDomainDescriptorForKey = ForestDomainViewDescriptor<Domain> && requires(const Domain domain, typename Domain::node_pptr p, const Key &key) {
{
domain.compare_key(key, p) }
-> std::convertible_to<int>;
};
template <typename Domain> static bool forest_domain_validate_node(const Domain &domain, typename Domain::node_pptr p) noexcept {
if constexpr (requires { { domain.validate_node(p) } -> std::convertible_to<bool>; }) return domain.validate_node(p);
else return true;
}
...
return domain.root_index();
}
template <typename Key> requires ForestDomainDescriptorForKey<Domain, Key> node_pptr find(const Key &key) const noexcept {
return avl_find<node_pptr>( domain.root_index(), [&](node_pptr cur) -> int { return domain.compare_key(key, cur); }, [this](node_pptr p) -> node_type * { return domain.resolve_node(p); });
}
};
...
if (root == nullptr || new_node.is_null()) return;
if (this->domain.resolve_node(new_node) == nullptr || !forest_domain_validate_node(this->domain, new_node)) return;
avl_insert( new_node, *root, [this, new_node](node_pptr cur) -> bool { return this->domain.less_node(new_node, cur); }, [this](node_pptr p) -> node_type * { return this->domain.resolve_node(p); });
}
};
...
return _idx == AddressTraitsT::no_block;
}
...
template <typename AddressTraitsT> static BlockPPtr<AddressTraitsT> pptr_make(BlockPPtr<AddressTraitsT> source, typename AddressTraitsT::index_type idx) noexcept {
...
if (_current_idx == static_cast<index_type>(0) || _current_idx == no_block) return NodePPtr();
return NodePPtr(_current_idx);
}
AvlInorderIterator &operator++() noexcept {
if (_current_idx == static_cast<index_type>(0) || _current_idx == no_block) return *this;
