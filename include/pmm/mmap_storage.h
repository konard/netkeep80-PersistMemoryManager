...
MMapStorage(MMapStorage &&other) noexcept : _base(other._base), _size(other._size), _mapped(other._mapped)
#if defined(_WIN32) || defined(_WIN64)
...
if (path == nullptr || size_bytes == 0) return false;
size_bytes = ((size_bytes + AddressTraitsT::granule_size - 1) / AddressTraitsT::granule_size) * AddressTraitsT::granule_size;
...
if (!_mapped || additional_bytes == 0) return _mapped && additional_bytes == 0;
...
_file_handle = CreateFileA(path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
...
if (view != nullptr) _base = static_cast<std::uint8_t *>(view);
}
return false;
}
...
void close_impl() noexcept {
if (_base != nullptr) ::munmap(_base, _size);
if (_fd >= 0) {
::close(_fd);
_fd = -1;
}
...
if (addr != MAP_FAILED) _base = static_cast<std::uint8_t *>(addr);
return false;
}
