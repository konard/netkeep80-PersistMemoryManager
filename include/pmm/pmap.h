...
const std::uint64_t traits = (std::uint64_t{std::is_integral_v<T>} << 0) | (std::uint64_t{std::is_floating_point_v<T>} << 1) | (std::uint64_t{std::is_signed_v<T>} << 2) | (std::uint64_t{std::is_unsigned_v<T>} << 3) | (std::uint64_t{std::is_pointer_v<T>} << 4) | (std::uint64_t{std::is_class_v<T>} << 5) | (std::uint64_t{std::is_enum_v<T>} << 6) | (std::uint64_t{std::is_trivially_copyable_v<T>} << 7) | (std::uint64_t{std::is_standard_layout_v<T>} << 8);
std::uint32_t h = 2166136261u;
h = pmap_fnv1a(h, sizeof(T), 8);
h = pmap_fnv1a(h, alignof(T), 8);
h = pmap_fnv1a(h, traits, 8);
for (const char *t = pmm::pmap_type_identity<T>::tag; t != nullptr && *t != '\0'; ++t) h = pmap_fnv1a(h, static_cast<std::uint8_t>(*t), 1);
return h;
}
...
inline bool pmap_write_name(char (&out)[kForestDomainNameCapacity], std::uint32_t type_fp, char kind, std::uint64_t value, unsigned value_hex_digits) noexcept {
constexpr const char *kPrefix = "container/pmap/";
const unsigned needed = 15 + 8 + 1 + 1 + value_hex_digits + 1;
if (needed > kForestDomainNameCapacity) return false;
std::size_t p = 0;
for (const char *s = kPrefix; *s != '\0'; ++s) out[p++] = *s;
...
static constexpr std::uint32_t domain_type_hash = detail::pmap_fnv1a( detail::pmap_fnv1a(2166136261u, detail::pmap_type_fp<_K>(), 4), detail::pmap_type_fp<_V>(), 4);
...
return ManagerT::forest_domain_root_index_unlocked( ManagerT::find_domain_by_binding_unlocked(binding_id));
}
...
private: index_type _binding_id;
...
do {
if (!detail::pmap_write_name(buf, domain_type_hash, 'g', seq++, 8)) return false;
}
...
public: pmap() noexcept : _binding_id(0) {
}
...
index_type root_index() const noexcept {
return descriptor().root_index();
}
forest_domain_policy forest_domain_ops() noexcept {
if (_binding_id == 0 || ManagerT::find_domain_by_binding_unlocked(_binding_id) == nullptr) bind(nullptr);
...
if (ops.root_index_ptr() == nullptr) return node_pptr();
node_pptr existing = ops.find(key);
if (!existing.is_null()) {
if (node_type *obj = ManagerT::template resolve<node_type>(existing); obj != nullptr) obj->value = val;
return existing;
}
...
if (obj == nullptr) return node_pptr();
obj->key = key;
obj->value = val;
...
if (*root != static_cast<index_type>(0)) detail::avl_clear_subtree(node_pptr(*root), [](node_pptr p) { ManagerT::template deallocate_typed<node_type>(p); });
*root = static_cast<index_type>(0);
}
void reset() noexcept {
forest_domain_policy(descriptor()).reset_root();
}
...
const index_type root = root_index();
if (root == static_cast<index_type>(0)) return iterator();
return iterator(detail::avl_min_node(node_pptr(root)).offset());
}
iterator end() const noexcept {
return iterator(static_cast<index_type>(0));
}
};
}
