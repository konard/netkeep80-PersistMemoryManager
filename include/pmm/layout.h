...
if (static_cast<std::size_t>(kFreeBlkIdx) * kGranSz + sizeof(Block<address_traits>) + kMinBlockDataSize > size) return false;
void *hdr_blk = base;
std::memset(hdr_blk, 0, ManagerAccess::kBlockHdrByteSize);
BlockState::init_fields(hdr_blk, address_traits::no_block, kFreeBlkIdx, 0, ManagerAccess::kMgrHdrGranules, kHdrBlkIdx);
...
if (data_gran_need == 0) data_gran_need = 1;
std::size_t min_need = static_cast<std::size_t>(ManagerAccess::kBlockHdrGranules + data_gran_need + ManagerAccess::kBlockHdrGranules) * kGranSz;
std::size_t growth = old_size / 4;
if (growth < min_need) growth = min_need;
if (!backend.expand(growth)) return false;
std::uint8_t *new_base = backend.base_ptr();
std::size_t new_size = backend.total_size();
if (new_base == nullptr || new_size <= old_size) return false;
...
void *last_blk_raw = (hdr->last_block_offset != address_traits::no_block) ? static_cast<void *>( new_base + static_cast<std::size_t>(hdr->last_block_offset) * kGranSz) : nullptr;
...
if (extra_size < sizeof(Block<address_traits>) + kGranSz) return false;
