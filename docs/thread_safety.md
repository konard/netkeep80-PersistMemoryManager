...
#### [SharedMutexLock](../include/pmm/config.h#pmm-config-sharedmutexlock)
...
#### [NoLock](../include/pmm/config.h#pmm-config-nolock)
...
Политика задаётся через шаблонный параметр [BasicConfig](../include/pmm/manager_configs.h#pmm-basicconfig):
...
|-------------|----------|-------------------|
| `CacheManagerConfig` | [NoLock](../include/pmm/config.h#pmm-config-nolock) | Временный кеш, однопоточный код |
| `EmbeddedManagerConfig` | [NoLock](../include/pmm/config.h#pmm-config-nolock) | Встраиваемые системы |
| `PersistentDataConfig` | [SharedMutexLock](../include/pmm/config.h#pmm-config-sharedmutexlock) | Долговременное хранение, файловые менеджеры |
| `IndustrialDBConfig` | [SharedMutexLock](../include/pmm/config.h#pmm-config-sharedmutexlock) | Промышленные базы данных |
| `LargeDBConfig` | [SharedMutexLock](../include/pmm/config.h#pmm-config-sharedmutexlock) | Крупные хранилища (64-bit индекс) |
...
При использовании [SharedMutexLock](../include/pmm/config.h#pmm-config-sharedmutexlock) они блокируют все остальные потоки.
...
2. **Стабильность базового адреса** — после инициализации `_backend.base_ptr()` не
   ...
   [HeapStorage](../include/pmm/heap_storage.h#pmm-heapstorage) (realloc может переместить буфер, но это происходит внутри
   защищённой секции).

3. **Возвращённый указатель валиден** — пока менеджер инициализирован и [pptr](../include/pmm/pptr.h#pmm-pptr) указывает
...
Персистентные контейнеры ([pstring](../include/pmm/pstring.h#pmm-pstring), [parray](../include/pmm/parray.h#pmm-parray), [pmap](../include/pmm/pmap.h#pmm-pmap)) **не имеют
...
- Используйте [NoLock](../include/pmm/config.h#pmm-config-nolock) (или `CacheManagerConfig`) для однопоточных приложений —
  нулевые накладные расходы на блокировку.
- Используйте [SharedMutexLock](../include/pmm/config.h#pmm-config-sharedmutexlock) (или `PersistentDataConfig`, `IndustrialDBConfig`)
  для многопоточных приложений.
- Все предустановки с [SharedMutexLock](../include/pmm/config.h#pmm-config-sharedmutexlock) поддерживают конкурентное чтение
