...
Этот документ фиксирует **каноническую прикладную архитектурную модель [PersistMemoryManager](../include/pmm/persist_memory_manager.h#pmm-persistmemorymanager)**.
...
[Block](../include/pmm/block.h#pmm-block) является базовым атомом PMM, потому что каждый блок одновременно несёт:
...
Префикс [BlockHeader](../include/pmm/block_header.h#pmm-blockheader) (`weight`, `left_offset`,
`right_offset`, `parent_offset`, `avl_height`) следует понимать как **универсальный
intrusive AVL-slot**. Это не отдельный объект `TreeNode`, а часть единственного
физического layout блока.
...
`Block<AT>` теперь является type alias для `BlockHeader<AT>`. Соотношение между блоком
и forest-доменом:

- сам [BlockHeader](../include/pmm/block_header.h#pmm-blockheader) связывает блок с линейным ПАП через `prev_offset`/`next_offset`;
- его AVL-slot префикс делает блок узлом одного forest-домена.
...
- forest-использование встроенных tree-полей в [pstringview](../include/pmm/pstringview.h#pmm-pstringview);
- forest-использование встроенных tree-полей в [pmap](../include/pmm/pmap.h#pmm-pmap), чей root хранится в type-scoped
...
- единственный физический layout блока [BlockHeader](../include/pmm/block_header.h#pmm-blockheader) и его AVL-slot префикс нужно объяснять через forest-модель, а не только через allocator.
