...
TEST_CASE( "pptr_tree_node_links", "[test_pptr]" )
{
    ...
    REQUIRE( node1.tree_node_unchecked().left_offset == no_block );
    REQUIRE( node1.tree_node_unchecked().right_offset == no_block );
    REQUIRE( node1.tree_node_unchecked().parent_offset == no_block );
    REQUIRE( node2.tree_node_unchecked().left_offset == no_block );
    REQUIRE( node2.tree_node_unchecked().right_offset == no_block );
    REQUIRE( node2.tree_node_unchecked().parent_offset == no_block );

    // Построить дерево: node1 — корень, node2 — левый, node3 — правый
    node1.tree_node_unchecked().left_offset   = node2.offset();
    node1.tree_node_unchecked().right_offset  = node3.offset();
    node2.tree_node_unchecked().parent_offset = node1.offset();
    node3.tree_node_unchecked().parent_offset = node1.offset();

    // Проверить связи
    REQUIRE( node1.tree_node_unchecked().left_offset == node2.offset() );
    REQUIRE( node1.tree_node_unchecked().right_offset == node3.offset() );
    REQUIRE( node2.tree_node_unchecked().parent_offset == node1.offset() );
    REQUIRE( node3.tree_node_unchecked().parent_offset == node1.offset() );

    // Потомки node2 и node3 по-прежнему null (no_block)
    REQUIRE( node2.tree_node_unchecked().left_offset == no_block );
    REQUIRE( node2.tree_node_unchecked().right_offset == no_block );
    REQUIRE( node3.tree_node_unchecked().left_offset == no_block );
    REQUIRE( node3.tree_node_unchecked().right_offset == no_block );
...
TEST_CASE( "pptr_tree_node_height", "[test_pptr]" )
{
    ...
    REQUIRE( node1.tree_node_unchecked().avl_height == 0 );
    REQUIRE( node2.tree_node_unchecked().avl_height == 0 );

    // Установить высоту
    node1.tree_node_unchecked().avl_height = 2;
    node2.tree_node_unchecked().avl_height = 1;

    REQUIRE( node1.tree_node_unchecked().avl_height == 2 );
    REQUIRE( node2.tree_node_unchecked().avl_height == 1 );
...
TEST_CASE( "pptr_tree_node_weight", "[test_pptr]" )
{
    ...
    REQUIRE( p.tree_node_unchecked().weight > 0 );
...
TEST_CASE( "pptr_user_avl_tree", "[test_pptr]" )
{
    ...
    *n5 = 25;
    ...
    n1.tree_node_unchecked().left_offset   = n2.offset();
    n1.tree_node_unchecked().right_offset  = n3.offset();
    n1.tree_node_unchecked().parent_offset = no_block;
    n1.tree_node_unchecked().avl_height    = 3;

    n2.tree_node_unchecked().left_offset   = n4.offset();
    n2.tree_node_unchecked().right_offset  = n5.offset();
    n2.tree_node_unchecked().parent_offset = n1.offset();
    n2.tree_node_unchecked().avl_height    = 2;

    n3.tree_node_unchecked().left_offset   = no_block;
    n3.tree_node_unchecked().right_offset  = no_block;
    n3.tree_node_unchecked().parent_offset = n1.offset();
    n3.tree_node_unchecked().avl_height    = 1;

    n4.tree_node_unchecked().left_offset   = no_block;
    n4.tree_node_unchecked().right_offset  = no_block;
    n4.tree_node_unchecked().parent_offset = n2.offset();
    n4.tree_node_unchecked().avl_height    = 1;

    n5.tree_node_unchecked().left_offset   = no_block;
    n5.tree_node_unchecked().right_offset  = no_block;
    n5.tree_node_unchecked().parent_offset = n2.offset();
    n5.tree_node_unchecked().avl_height    = 1;

    // Проверить структуру дерева
    REQUIRE( n1.tree_node_unchecked().left_offset == n2.offset() );
    REQUIRE( n1.tree_node_unchecked().right_offset == n3.offset() );
    REQUIRE( n1.tree_node_unchecked().parent_offset == no_block );
    REQUIRE( n1.tree_node_unchecked().avl_height == 3 );

    REQUIRE( n2.tree_node_unchecked().left_offset == n4.offset() );
    REQUIRE( n2.tree_node_unchecked().right_offset == n5.offset() );
    REQUIRE( n2.tree_node_unchecked().parent_offset == n1.offset() );
    REQUIRE( n2.tree_node_unchecked().avl_height == 2 );

    REQUIRE( n3.tree_node_unchecked().left_offset == no_block );
    REQUIRE( n3.tree_node_unchecked().right_offset == no_block );
    REQUIRE( n3.tree_node_unchecked().parent_offset == n1.offset() );
    REQUIRE( n3.tree_node_unchecked().avl_height == 1 );
...
TEST_CASE( "pptr_tree_node_ref", "[test_pptr]" )
{
    ...
    auto& tn1 = n1.tree_node_unchecked();
    auto& tn2 = n2.tree_node_unchecked();
    auto& tn3 = n3.tree_node_unchecked();
